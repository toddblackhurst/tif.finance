...
export default async function BankCashFlowPage({
  params,
  searchParams,
}: {
  ...
  const lines = (rawLines ?? []) as BankCashFlowLine[];
  const moneyIn = lines.filter((line) => line.amount > 0).reduce((sum, line) => sum + line.amount, 0);
  const moneyOut = lines.filter((line) => line.amount < 0).reduce((sum, line) => sum + Math.abs(line.amount), 0);
  const net = moneyIn - moneyOut;
  const unmatchedCount = lines.filter((line) => line.match_status === "unmatched").length;

  return (
    <div className="space-y-6 max-w-6xl">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="space-y-1">
          <Link href={`/${locale}/bank`} className="text-sm text-gray-500 hover:text-gray-700">
            &larr; Bank Transactions
          </Link>
          <h1 className="text-2xl font-bold">Bank Cash Flow</h1>
        </div>
        <form className="flex items-center gap-2" action={`/${locale}/bank/cash-flow`}>
          <input
            type="month"
            name="month"
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <div className="rounded-lg border bg-white p-4">
          <p className="text-xs font-medium uppercase tracking-wide text-green-700">Money In</p>
          <p className="mt-1 text-xl font-bold text-green-700">{fmt(moneyIn)}</p>
        </div>
        <div className="rounded-lg border bg-white p-4">
          <p className="text-xs font-medium uppercase tracking-wide text-red-700">Money Out</p>
          <p className="mt-1 text-xl font-bold text-red-700">{fmt(moneyOut)}</p>
        </div>
        <div className="rounded-lg border bg-white p-4">
          ...
          {unmatchedCount > 0 && (
            <p className="mt-1 text-xs font-medium text-amber-700">{unmatchedCount} unmatched</p>
          )}
        </div>
      </div>
