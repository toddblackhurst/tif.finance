...
export async function sendExpenseSubmittedEmail({
  ...
  await resend.emails.send({
    from: FROM,
    to: approverEmails,
    subject: `[TIF Finance] Expense needs approval — ${fmt(amount)} · ${campus}`,
    html: `
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:520px;margin:0 auto;color:#222;">
        ...
        <p style="color:#555;margin-top:0;">Hi ${submitterName}, your expense has been approved.</p>
        <table style="width:100%;border-collapse:collapse;margin:16px 0;font-size:14px;">
          <tr><td style="padding:6px 0;color:#666;width:120px;">Description</td><td><strong>${description}</strong></td></tr>
        ...
        <a href="${link}" style="display:inline-block;background:#15803d;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:600;">
          View Expense →
        </a>
        <p style="margin-top:24px;font-size:12px;color:#999;">TIF Finance · Internal use only</p>
      </div>
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:520px;margin:0 auto;color:#222;">
        ...
        <p style="color:#555;margin-top:0;">Hi ${submitterName}, your expense was not approved.</p>
        <table style="width:100%;border-collapse:collapse;margin:16px 0;font-size:14px;">
          <tr><td style="padding:6px 0;color:#666;width:120px;">Description</td><td><strong>${description}</strong></td></tr>
          <tr><td style="padding:6px 0;color:#666;">Amount</td><td><strong>${fmt(amount)}</strong></td></tr>
          <tr><td style="padding:6px 0;color:#666;">Reviewed by</td><td>${approverName}</td></tr>
          ${rejectionNote ? `<tr><td style="padding:6px 0;color:#666;">Note</td><td>${rejectionNote}</td></tr>` : ""}
        </table>
        <a href="${link}" style="display:inline-block;background:#6b7280;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:600;">
          View Expense →
        </a>
        <p style="margin-top:24px;font-size:12px;color:#999;">TIF Finance · Internal use only</p>
      </div>
